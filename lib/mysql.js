...
mysql.selectAttendanceMessage = function (callback){
	var data = [];
	var facebookurl="";
	var timeline_attendance = connection.query("SELECT  id,Confirmation,Attendance,Friends,NAME as nm,concat(concat(TRIM(SUBSTRING(NAME,LOCATE( ' ', NAME ))),concat(' ',TRIM(SUBSTRING(NAME,1,LOCATE( ' ', NAME )-1)))),concat('(',concat(concat(Attendance,concat('/',Friends)),')'))) as Name,Email,Phone,Message,PicturePath,FacebookId,CHAR_LENGTH(PicturePath) as piclen,CHAR_LENGTH(FacebookId) as idlen,Date,(select count(*) from wedding.attendance where attendance='yes')as plan,(select count(*) from wedding.attendance where attendance='yes' and confirmation='done') as actual,Gender,team  FROM wedding.attendance  order by confirmation desc,team,Name asc",function (error, results, fields) {
		//console.log(results);
		if (error) throw err;
		async.each(results, function(timeline, callback){
		if(timeline.idlen > 1){
